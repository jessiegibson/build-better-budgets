...
def new_budget():
    ...
    if request.method == 'GET':
        ...
        return render_template('new_budget.html', categories=categories)
...
def analyze():
    ...
    three_months_ago = (datetime.datetime.strptime(max_date, '%Y-%m-%d') - datetime.timedelta(days=90)).strftime('%Y-%m-%d')
    ...
    top_categories = [dict(row) for row in cursor.fetchone()]
