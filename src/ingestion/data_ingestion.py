...
from src.ingestion.pdf_extractor import extract_transactions_from_pdf, pdf_to_csv, process_pdf_directory
from src.ingestion.qfx_importer import parse_qfx_file, qfx_to_csv, process_qfx_directory
    ...
    for csv_path in csv_files:
        try:
            ...
            if 'source' in df.columns:
                source_name = df['source'].iloc[0] if not df.empty else os.path.basename(csv_path)
            else:
            ...
            for _, row in df.iterrows():
                try:
                    cursor.execute('''
                        INSERT INTO consolidated_transactions 
                        (date_posted, description, amount, details, source)
                        VALUES (?, ?, ?, ?, ?)
                    ''', (
                        row['date'] if 'date' in row and pd.notna(row['date']) else None,
                        row['description'] if 'description' in row and pd.notna(row['description']) else '',
                        row['amount'] if 'amount' in row and pd.notna(row['amount']) else 0,
                        row['details'] if 'details' in row and pd.notna(row['details']) else '',
                        source_name
                    ))
                    record_count += 1
            ...
            # Record import in result
            if source_name in result:
                result[source_name] += record_count
            else:
                result[source_name] = record_count
            ...
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM import_log WHERE source = ? AND filename = ?", (source_name, filename))
            import_record = cursor.fetchone()
            ...
            if import_record:
                ...
                cursor.execute('''
                    INSERT INTO import_log (source, filename, import_date, record_count)
                    VALUES (?, ?, ?, ?)
                ''', (source_name, filename, import_date, record_count))
            
            conn.commit()
            conn.close()
            
            logger.info(f"Imported {record_count} records from {csv_path}")
...
def get_transaction_summary() -> Dict[str, Any]:
    ...
    cursor.execute("""
        SELECT 
            COALESCE(SUM(CASE WHEN rental_related = 1 THEN 1 ELSE 0 END), 0) as rental_count,
            COALESCE(SUM(CASE WHEN recurring = 1 THEN 1 ELSE 0 END), 0) as recurring_count,
            COALESCE(SUM(CASE WHEN subscription = 1 THEN 1 ELSE 0 END), 0) as subscription_count,
            COALESCE(SUM(CASE WHEN tax_deductible = 1 THEN 1 ELSE 0 END), 0) as tax_deductible_count,
            COALESCE(SUM(CASE WHEN equipment_tools = 1 THEN 1 ELSE 0 END), 0) as equipment_count
        FROM consolidated_transactions
    """)
